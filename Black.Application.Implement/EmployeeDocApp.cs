...
{
    public class EmployeeDocApp : AppBaseCRUD<EmployeeDoc, Guid>, IEmployeeDocApp
    {
        #region Properties
        private readonly IEmployeeDocService _mentoredDocService;
        private readonly IAzureStorageService _azureStorageService;
        private readonly IConfiguration _configuration;
        #endregion

        public EmployeeDocApp(IEmployeeDocService service, INotificationHandler<DomainNotification> notification, IUser user, IUnitOfWork uow,
            IAzureStorageService azureStorageService, IConfiguration configuration, IEmployeeDocService EmployeeDocService) : base(service, notification, user, uow)
        {
            _azureStorageService = azureStorageService;
            _configuration = configuration;
            _mentoredDocService = EmployeeDocService;
        }

        public async Task<EmployeeDoc> UploadDocAsync(int mentoredId, TypeDocEnum typeDoc, Stream filestream = null, string filename = null)
        {
            ...
            string container = _configuration["Integration:AzureStorageBlob:EmployeeDoc-Container"];
            // Cria um novo doc
            var doc = new EmployeeDoc(filename, extension, typeDoc, container, mentoredId);
        ...
        public async Task<EmployeeDoc> ChangeStatusDoc(Guid docId, int mentoredId, TypeDocEnum typeDoc, bool active)
        {
        ...
        public Task<List<EmployeeDoc>> GetAllByEmployeeIdAsync(int mentoredId) => _mentoredDocService.GetAllByEmployeeIdAsync(mentoredId);

        public Task<List<EmployeeDoc>> GetAllActivesAsync(int mentoredDependentId) => _mentoredDocService.GetAllActives(mentoredDependentId);
