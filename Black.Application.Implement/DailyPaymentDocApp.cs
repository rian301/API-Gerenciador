...
{
    public class DailyPaymentDocApp : AppBaseCRUD<DailyPaymentDoc, Guid>, IDailyPaymentDocApp
    {
        #region Properties
        private readonly IDailyPaymentDocService _mentoredDocService;
        private readonly IAzureStorageService _azureStorageService;
        private readonly IConfiguration _configuration;
        #endregion

        public DailyPaymentDocApp(IDailyPaymentDocService service, INotificationHandler<DomainNotification> notification, IUser user, IUnitOfWork uow,
            IAzureStorageService azureStorageService, IConfiguration configuration, IDailyPaymentDocService DailyPaymentDocService) : base(service, notification, user, uow)
        {
            _azureStorageService = azureStorageService;
            _configuration = configuration;
            _mentoredDocService = DailyPaymentDocService;
        }

        public async Task<DailyPaymentDoc> UploadDocAsync(int mentoredId, TypeDocEnum type, Stream filestream = null, string filename = null)
        {
            ...
            string container = _configuration["Integration:AzureStorageBlob:DailyPaymentControlDoc-Container"];
            // Cria um novo doc
            var doc = new DailyPaymentDoc(filename, extension, type, container, mentoredId);
        ...
        public async Task<DailyPaymentDoc> ChangeStatusDoc(Guid docId, int mentoredId, TypeDocEnum typedoc, bool active)
        {
        ...
        public Task<List<DailyPaymentDoc>> GetAllByDailyPaymentIdAsync(int mentoredId) => _mentoredDocService.GetAllByDailyPaymentIdAsync(mentoredId);

        public Task<List<DailyPaymentDoc>> GetAllActivesAsync(int mentoredDependentId) => _mentoredDocService.GetAllActives(mentoredDependentId);
