...
{
    public class ExpenseControlDocApp : AppBaseCRUD<ExpenseControlDoc, Guid>, IExpenseControlDocApp
    {
        #region Properties
        private readonly IExpenseControlDocService _mentoredDocService;
        private readonly IAzureStorageService _azureStorageService;
        private readonly IConfiguration _configuration;
        #endregion

        public ExpenseControlDocApp(IExpenseControlDocService service, INotificationHandler<DomainNotification> notification, IUser user, IUnitOfWork uow,
            IAzureStorageService azureStorageService, IConfiguration configuration, IExpenseControlDocService ExpenseControlDocService) : base(service, notification, user, uow)
        {
            _azureStorageService = azureStorageService;
            _configuration = configuration;
            _mentoredDocService = ExpenseControlDocService;
        }

        public async Task<ExpenseControlDoc> UploadDocAsync(int mentoredId, TypeDocEnum type, Stream filestream = null, string filename = null)
        {
            ...
            string container = _configuration["Integration:AzureStorageBlob:ExpenseControlDoc-Container"];
            // Cria um novo doc
            var doc = new ExpenseControlDoc(filename, extension, type, container, mentoredId);
        ...
        public async Task<ExpenseControlDoc> ChangeStatusDoc(Guid docId, int mentoredId, TypeDocEnum typedoc, bool active)
        {
        ...
        public Task<List<ExpenseControlDoc>> GetAllByExpenseControlIdAsync(int mentoredId) => _mentoredDocService.GetAllByExpenseControlIdAsync(mentoredId);

        public Task<List<ExpenseControlDoc>> GetAllActivesAsync(int mentoredDependentId) => _mentoredDocService.GetAllActives(mentoredDependentId);
