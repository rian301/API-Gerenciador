...
{
    public class PurchaseControlDocApp : AppBaseCRUD<PurchaseControlDoc, Guid>, IPurchaseControlDocApp
    {
        #region Properties
        private readonly IPurchaseControlDocService _mentoredDocService;
        private readonly IAzureStorageService _azureStorageService;
        private readonly IConfiguration _configuration;
        #endregion

        public PurchaseControlDocApp(IPurchaseControlDocService service, INotificationHandler<DomainNotification> notification, IUser user, IUnitOfWork uow,
            IAzureStorageService azureStorageService, IConfiguration configuration, IPurchaseControlDocService PurchaseControlDocService) : base(service, notification, user, uow)
        {
            _azureStorageService = azureStorageService;
            _configuration = configuration;
            _mentoredDocService = PurchaseControlDocService;
        }

        public async Task<PurchaseControlDoc> UploadDocAsync(int mentoredId, TypeDocEnum type, Stream filestream = null, string filename = null)
        {
            ...
            string container = _configuration["Integration:AzureStorageBlob:PurchaseControl-Container"];
            // Cria um novo doc
            var doc = new PurchaseControlDoc(filename, extension, type, container, mentoredId);
        ...
        public async Task<PurchaseControlDoc> ChangeStatusDoc(Guid docId, int mentoredId, TypeDocEnum typedoc, bool active)
        {
        ...
        public Task<List<PurchaseControlDoc>> GetAllByPurchaseControlIdAsync(int mentoredId) => _mentoredDocService.GetAllByPurchaseControlIdAsync(mentoredId);

        public Task<List<PurchaseControlDoc>> GetAllActivesAsync(int mentoredDependentId) => _mentoredDocService.GetAllActives(mentoredDependentId);
        ...
        public async Task<BlobFileDTO> DownloadDoc(Guid id)
        {
            ...
            string container = _configuration["Integration:AzureStorageBlob:PurchaseControlDoc-Container"];
