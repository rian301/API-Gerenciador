...
{
    public class PatrimonyDocApp : AppBaseCRUD<PatrimonyDoc, Guid>, IPatrimonyDocApp
    {
        #region Properties
        private readonly IPatrimonyDocService _mentoredDocService;
        private readonly IAzureStorageService _azureStorageService;
        private readonly IConfiguration _configuration;
        #endregion

        public PatrimonyDocApp(IPatrimonyDocService service, INotificationHandler<DomainNotification> notification, IUser user, IUnitOfWork uow,
            IAzureStorageService azureStorageService, IConfiguration configuration, IPatrimonyDocService PatrimonyDocService) : base(service, notification, user, uow)
        {
            _azureStorageService = azureStorageService;
            _configuration = configuration;
            _mentoredDocService = PatrimonyDocService;
        }

        public async Task<PatrimonyDoc> UploadDocAsync(int mentoredId, TypeDocEnum type, Stream filestream = null, string filename = null)
        {
            ...
            string container = _configuration["Integration:AzureStorageBlob:PatrimonyDoc-Container"];
            // Cria um novo doc
            var doc = new PatrimonyDoc(filename, extension, type, container, mentoredId);
        ...
        public async Task<PatrimonyDoc> ChangeStatusDoc(Guid docId, int mentoredId, TypeDocEnum typedoc, bool active)
        {
        ...
        public Task<List<PatrimonyDoc>> GetAllByPatrimonyIdAsync(int mentoredId) => _mentoredDocService.GetAllByPatrimonyIdAsync(mentoredId);

        public Task<List<PatrimonyDoc>> GetAllActivesAsync(int mentoredDependentId) => _mentoredDocService.GetAllActives(mentoredDependentId);
